...
export interface ResourceNodeSnapshot { id: string; kind: 'copper_vein' | 'river_pool'; position: Position; available: boolean; respawnAt: number | null; }
export type StationKind = 'furnace' | 'anvil' | 'campfire';
...
export type ServiceSnapshot = { id: string; kind: 'bank'; position: Position } | { id: string; kind: 'merchant'; position: Position; offers: MerchantOfferSnapshot[] };
...
export interface PlayerProgressSnapshot {
  inventory: InventorySnapshot; bank: BankSnapshot; wallet: { coins: number }; equipment: { toolItemId: string | null };
  skills: { mining: SkillSnapshot; smithing: SkillSnapshot; fishing: SkillSnapshot; cooking: SkillSnapshot };
  gathering: GatheringSnapshot | null; processing: ProcessingSnapshot | null;
  worldFlags: { northernRoadOpen: boolean }; discoveries: string[];
}
export interface EnemySnapshot { id: string; kind: 'reach_rat' | 'road_wolf'; position: Position; health: number; maxHealth: number; alive: boolean; respawnAt: number | null; }
export interface CombatProgressSnapshot { health: { current: number; max: number; dead: boolean; respawnAt: number | null }; skill: SkillSnapshot; equipment: { weaponItemId: string | null }; }
...
export interface UseItemMessage { type: 'USE_ITEM'; sequence: number; itemId: string; }
...
export interface WelcomeMessage {
  type: 'WELCOME'; protocolVersion: typeof PROTOCOL_VERSION; serverBuild: string; connectionId: string; resumeToken: string; worldId: string;
  player: PlayerSnapshot; players: PlayerSnapshot[]; resources: ResourceNodeSnapshot[]; stations: StationSnapshot[]; services: ServiceSnapshot[];
  progress: PlayerProgressSnapshot; enemies: EnemySnapshot[]; combat: CombatProgressSnapshot; world: { bounds: WorldBounds };
}
...
export interface ActionRejectedMessage {
  ...
  action: 'movement' | 'gathering' | 'processing' | 'equipment' | 'consumable' | 'bank' | 'merchant' | 'combat';
  reason: 'invalid_target' | 'too_far' | 'node_unavailable' | 'inventory_full' | 'already_busy' | 'not_gathering' | 'not_processing' | 'invalid_recipe' | 'wrong_station' | 'missing_items' | 'item_not_owned' | 'invalid_equipment' | 'invalid_service' | 'invalid_quantity' | 'bank_full' | 'bank_missing_item' | 'insufficient_coins' | 'item_not_traded' | 'transaction_failed' | 'target_dead' | 'player_dead' | 'cooldown' | 'full_health' | 'route_locked';
}
...
export function createUseItem(sequence: number, itemId: string): UseItemMessage { return { type: 'USE_ITEM', sequence, itemId }; }
...
export function parseServerMessage(raw: string): ServerMessage {
  ...
  if (value.type === 'REJECT') { if ((value.reason !== 'protocol_mismatch' && value.reason !== 'invalid_message') || value.supportedProtocolVersion !== PROTOCOL_VERSION) throw new Error('Malformed REJECT message'); return value as unknown as RejectMessage; }
  if (value.type === 'ACTION_REJECTED') {
    const actions = ['movement', 'gathering', 'processing', 'equipment', 'consumable', 'bank', 'merchant', 'combat'];
    const reasons = ['invalid_target', 'too_far', 'node_unavailable', 'inventory_full', 'already_busy', 'not_gathering', 'not_processing', 'invalid_recipe', 'wrong_station', 'missing_items', 'item_not_owned', 'invalid_equipment', 'invalid_service', 'invalid_quantity', 'bank_full', 'bank_missing_item', 'insufficient_coins', 'item_not_traded', 'transaction_failed', 'target_dead', 'player_dead', 'cooldown', 'full_health', 'route_locked'];
  ...
  if (value.type === 'PLAYER_STATE') { if (!Number.isSafeInteger(value.revision) || !isProgress(value.progress)) throw new Error('Malformed PLAYER_STATE message'); return value as unknown as PlayerStateMessage; }
  if (value.type === 'COMBAT_PLAYER_STATE') { if (!Number.isSafeInteger(value.revision) || !isCombatProgress(value.combat)) throw new Error('Malformed COMBAT_PLAYER_STATE message'); return value as unknown as CombatPlayerStateMessage; }
  if (value.type === 'WORLD_STATE') { if (!Number.isSafeInteger(value.revision) || !isPlayers(value.players) || !isResources(value.resources) || !isStations(value.stations) || !isServices(value.services)) throw new Error('Malformed WORLD_STATE message'); return value as unknown as WorldStateMessage; }
  if (value.type === 'COMBAT_WORLD_STATE') { if (!Number.isSafeInteger(value.revision) || !isEnemies(value.enemies)) throw new Error('Malformed COMBAT_WORLD_STATE message'); return value as unknown as CombatWorldStateMessage; }
  if (value.type !== 'WELCOME') throw new Error(`Unknown server message type: ${value.type}`);
  if (value.protocolVersion !== PROTOCOL_VERSION || typeof value.serverBuild !== 'string' || typeof value.connectionId !== 'string' || typeof value.resumeToken !== 'string' || typeof value.worldId !== 'string' || !isPlayer(value.player) || !isPlayers(value.players) || !isResources(value.resources) || !isStations(value.stations) || !isServices(value.services) || !isProgress(value.progress) || !isEnemies(value.enemies) || !isCombatProgress(value.combat) || !isRecord(value.world) || !isBounds(value.world.bounds)) throw new Error('Malformed WELCOME message');
...
function isResource(value: unknown): value is ResourceNodeSnapshot { return isRecord(value) && typeof value.id === 'string' && (value.kind === 'copper_vein' || value.kind === 'river_pool') && isPosition(value.position) && typeof value.available === 'boolean' && (value.respawnAt === null || typeof value.respawnAt === 'number'); }
function isResources(value: unknown): value is ResourceNodeSnapshot[] { return Array.isArray(value) && value.every(isResource); }
function isStation(value: unknown): value is StationSnapshot { return isRecord(value) && typeof value.id === 'string' && (value.kind === 'furnace' || value.kind === 'anvil' || value.kind === 'campfire') && isPosition(value.position); }
...
function isService(value: unknown): value is ServiceSnapshot { if (!isRecord(value) || typeof value.id !== 'string' || !isPosition(value.position)) return false; if (value.kind === 'bank') return true; return value.kind === 'merchant' && Array.isArray(value.offers) && value.offers.every(isMerchantOffer); }
...
function isProgress(value: unknown): value is PlayerProgressSnapshot {
  if (!isRecord(value) || !isRecord(value.inventory) || !isRecord(value.bank) || !isRecord(value.wallet) || !isRecord(value.equipment) || !isRecord(value.skills) || !isRecord(value.worldFlags)) return false;
  ...
  if (!isSkill(value.skills.mining) || !isSkill(value.skills.smithing) || !isSkill(value.skills.fishing) || !isSkill(value.skills.cooking)) return false;
  if (typeof value.worldFlags.northernRoadOpen !== 'boolean' || !Array.isArray(value.discoveries) || !value.discoveries.every((item) => typeof item === 'string')) return false;
...
function isEnemy(value: unknown): value is EnemySnapshot { return isRecord(value) && typeof value.id === 'string' && (value.kind === 'reach_rat' || value.kind === 'road_wolf') && isPosition(value.position) && Number.isSafeInteger(value.health) && (value.health as number) >= 0 && Number.isSafeInteger(value.maxHealth) && (value.maxHealth as number) > 0 && typeof value.alive === 'boolean' && (value.respawnAt === null || typeof value.respawnAt === 'number'); }
function isEnemies(value: unknown): value is EnemySnapshot[] { return Array.isArray(value) && value.every(isEnemy); }
function isCombatProgress(value: unknown): value is CombatProgressSnapshot { if (!isRecord(value) || !isRecord(value.health) || !isRecord(value.equipment)) return false; if (!Number.isSafeInteger(value.health.current) || (value.health.current as number) < 0 || !Number.isSafeInteger(value.health.max) || (value.health.max as number) <= 0) return false; if (typeof value.health.dead !== 'boolean' || !(value.health.respawnAt === null || typeof value.health.respawnAt === 'number')) return false; if (!isSkill(value.skill)) return false; return value.equipment.weaponItemId === null || typeof value.equipment.weaponItemId === 'string'; }
