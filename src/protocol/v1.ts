...
export interface WorldStateMessage { type: 'WORLD_STATE'; revision: number; players: PlayerSnapshot[]; resources: ResourceNodeSnapshot[]; stations: StationSnapshot[]; }
...
export interface ActionRejectedMessage {
  ...
  action: 'movement' | 'gathering' | 'processing' | 'equipment';
  reason: 'invalid_target' | 'too_far' | 'node_unavailable' | 'inventory_full' | 'already_busy' | 'not_gathering' | 'not_processing' | 'invalid_recipe' | 'wrong_station' | 'missing_items' | 'item_not_owned' | 'invalid_equipment';
}
...
export function createHello(clientBuild: string, resumeToken?: string): HelloMessage {
  return { type: 'HELLO', protocolVersion: PROTOCOL_VERSION, clientBuild, ...(resumeToken ? { resumeToken } : {}) };
}
...
export function parseServerMessage(raw: string): ServerMessage {
  ...
  if (value.type === 'ACTION_REJECTED') {
    const actions = ['movement', 'gathering', 'processing', 'equipment'];
    const reasons = ['invalid_target', 'too_far', 'node_unavailable', 'inventory_full', 'already_busy', 'not_gathering', 'not_processing', 'invalid_recipe', 'wrong_station', 'missing_items', 'item_not_owned', 'invalid_equipment'];
  ...
  if (value.type === 'WORLD_STATE') {
    if (!Number.isSafeInteger(value.revision) || !isPlayers(value.players) || !isResources(value.resources) || !isStations(value.stations)) throw new Error('Malformed WORLD_STATE message');
  ...
  if (
    ...
    !isPlayer(value.player) || !isPlayers(value.players) || !isResources(value.resources) || !isStations(value.stations) ||
...
function isResource(value: unknown): value is ResourceNodeSnapshot {
  return isRecord(value) && typeof value.id === 'string' && value.kind === 'copper_vein' && isPosition(value.position) && typeof value.available === 'boolean' && (value.respawnAt === null || typeof value.respawnAt === 'number');
}
...
function isProgress(value: unknown): value is PlayerProgressSnapshot {
  if (!isRecord(value) || !isRecord(value.inventory) || !isRecord(value.equipment) || !isRecord(value.skills)) return false;
